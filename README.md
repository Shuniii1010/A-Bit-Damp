...
The original responses are preserved unchanged in `data/`. Of the 28 locations, 26 have complete humidity values for the selected times. 

Tai Mei Tuk and Yuen Long Park are excluded because their humidity values are missing, rather than being treated as zero.
...
The animation contains 12 frames, sampling the forecast every two hours from 00:00 to 22:00 Hong Kong time (UTC+8). Each ink spot is positioned using its station’s geographical coordinates. 

Higher humidity produces darker ink and a wider spread. Each station keeps the same underlying texture throughout the animation, so changes in appearance reflect the data rather than randomly changing shapes.
